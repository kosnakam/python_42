...
def apply_limit(bmi: list[int | float], limit: int) -> list[bool]:
    try:
        assert type(limit) == int, "The second argument must be of type int!"
        assert all(isinstance(element, (float, int)) for element in bmi), "Height lists of arguments has to have int or float!"
